...
const Input = forwardRef<InputRef,InputProps>(({type, name, placeholder, icon, onChangeCheckBox, check, disable, validation}, ref) =>{
    ...
    return (
        ...
        type === "text"  || type === "password" || type === "email" || type === "date"  ? 
            ...
            <div className="otp-input" data-state={!isValid ? 'bad' : 'good'}> <input  type="text" value={value} onChange={onChange} {...(disable ? {disabled: true}: {})} maxLength={1} placeholder=""  autoComplete="off" /> </div> : ""
        }
        </>
    )
