...
type ErrorInputValue = {
  ...
  password: "Min, maj, chiffre, symbole et +8 caractères"
  date: "Date invalide"
...
const Input = forwardRef<InputRef,InputProps>(({type, name, placeholder, icon, onChangeCheckBox, check, disable, validation}, ref) =>{
    ...
    const otpChange = (e:React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (validateInput(e.target.value)) {
            ...
            if (next) {
                ...
                const btnoOtp = e.target.parentElement?.parentElement?.parentElement?.children[2].children[0] as HTMLButtonElement
                btnoOtp.removeAttribute('disabled')
            }
    ...
    const onKey = (e:React.KeyboardEvent<HTMLInputElement>) => {
        const btnoOtp = e.currentTarget.parentElement?.parentElement?.parentElement?.children[2].children[0] as HTMLButtonElement
        btnoOtp.setAttribute('disabled','')
    ...
    return (
        <>
        {
            type === "text"  || type === "password" || type === "email" || type === "date" || type === "number" ? 
            <div className="ui-input" data-name={name} data-error={!isValid ? errorMessages[type]: ''} data-state={!isValid ? 'bad' : 'good'}>
                ...
                <input type={changeType()} placeholder={placeholder} id={userId} autoComplete="off" value={value} onChange={onChange}/> 
                {type === 'password' && (<button type="button" data-active={showPassword ? "true": ""} onClick={viewPassword}>
                                            <i className="fa-solid fa-eye" style={{display: showPassword ? "none": "block"}}></i>
                                            <i className="fa-solid fa-eye-slash" style={{display: showPassword ? "block": "none"}} ></i>
                                        </button>)}
            </div> : type === "phone" ?
