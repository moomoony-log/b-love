...
const Input = forwardRef<InputRef,InputProps>(({type, name, placeholder, icon, onChangeCheckBox, check, disable, validation}, ref) =>{
    ...
    const onChange = (e:React.ChangeEvent<HTMLInputElement>) => {
        ...
        if(type === 'otp'){
            if(validateInput(inputValue)){
                const next = (e.target.parentElement?.nextElementSibling as HTMLDivElement)?.querySelector("input") 
                if (next) {
                    console.log(next);
                    setValue(inputValue)
    ...
    return (
        ...
        type === "text"  || type === "password" || type === "email" || type === "date"  ? 
            ...
            <div className="otp-input"> <input  type="text" value={value} onChange={onChange} {...(disable ? {disabled: true}: {})} maxLength={1} placeholder=""  autoComplete="off" /> </div> : ""
        }
        </>
    )
