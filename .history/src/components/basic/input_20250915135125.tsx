...
const Input = forwardRef<InputRef,InputProps>(({type, name, placeholder, icon, onChangeCheckBox, check, disable, validation}, ref) =>{
    ...
    return (
        <>
        {
            ...
            <div className="ui-input" data-name={name} data-error={isValid ? errorMessages[type]: ''} data-state={isValid ? '' : 'good'}>
            ...
            <div className="otp-input" data-state={isValid ? 'good' : 'bad'}> <input type="text" value={value} onChange={otpChange} onKeyDown={onKey}  {...(disable ? {disabled: true}: {})} maxLength={1} placeholder="" autoComplete="off"/></div> : ""
        }
        </>
    )
